...
class CloudCollector(object):
    ...
    # Some collector can use token that expires within some time limit. Thus such token can be
    # cached to some file
    TOKEN_CACHE_FILE = None

    # When token is supported by cloud provider, then this value is in seconds
